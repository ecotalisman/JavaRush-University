...
public class Team implements Callable<Integer> {
    ...
    private boolean stakeOutTrack() {
        ...
        if (nonNull(track)) {
            System.out.printf("Команда '%s' застовбила доріжку №%d за $%d\n", title, track.getNumber(), track.getPrice());
            return true;
        }
        System.out.printf("Для команди '%s' не знайшлося вільної доріжки\n", title);
        return false;
    }
    ...
    private boolean getShoes() {
        ...
        if (nonNull(shoes)) {
            ...
            System.out.printf("Команда '%s' перевзулась\n", title);
            return true;
        }
        System.out.printf("Команді '%s' не вистачило взуття\n", title);
    ...
    private int play() {
        ...
        System.out.printf("Команда '%s' ганяє кулі, замовляє їжу та випивку на $%d\n", title, spentMoney);
        try {
